...
export const archNodes: ArchNode[] = [
  {
    ...
    type: 'client',
    description: 'The frontend user interface (SPA, iOS/Android) initiating OAuth, login, and token refresh requests.',
    responsibility: [
    ...
    configTip: 'Avoid storing tokens in localStorage or sessionStorage to protect against Cross-Site Scripting (XSS) attacks.'
  },
  {
    id: 'gateway',
    label: 'Reverse Proxy & Rate Limiter',
    type: 'network',
    description: 'Nginx, Cloudflare or AWS API Gateway protecting the internal microservices, performing rate limiting, and terminating SSL.',
    responsibility: [
    ...
    configTip: 'Configure maximum request limits, and deny requests from suspicious IP address spaces. Route traffic over private VPC.'
  },
  {
    id: 'express_app',
    label: 'FortifyAuth Server (Express.js)',
    type: 'server',
    description: 'The main, stateless authenticating engine written in TypeScript and Express, handling routes, hash validations, JWT creation, and session management.',
    responsibility: [
    ...
    configTip: 'Use a custom Global Error Handler that catch-alls errors, logs full traces internally, and returns sterile, generic warnings to users.'
  },
  {
    id: 'redis_store',
    label: 'Redis Session & Cache Store',
    type: 'database',
    description: 'High-speed, in-memory data store holding rate limit bucket states, active sessions, and blacklisted JWT blocks.',
    responsibility: [
    ...
    configTip: 'Configure evictions as volatile-lru to ensure rate limiter and blacklists expire elements automatically according to TTL thresholds.'
  },
  {
    id: 'postgres_db',
    label: 'PostgreSQL Relational DB',
    type: 'database',
    description: 'Core ACID-compliant database holding normalized tables for users, passwords, sessions, login logs, and audit trails.',
    responsibility: [
    ...
    configTip: 'Index fields targeted frequently for lookup, such as User.email, Session.tokenHash, and AuditLog.userId to maximize performance.'
  },
  {
    id: 'smtp_server',
    label: 'SMTP Email Service Provider',
    type: 'external',
    description: 'External transactional mail delivery service to dispatch password reset coordinates and registration verification pins.',
    responsibility: [
    ...
    configTip: 'Configure SMTP with strict SPF, DKIM, and DMARC parameters on your sending domain to prevent email spoofing and ensure clean delivery.'
  }
];
