...
import { Shovel as LogActivity, Search, Filter, ShieldCheck, ShieldAlert, KeyRound, Info } from 'lucide-react';
...
export default function AuditTimelineView() {
  ...
  const logs: AuditItem[] = [
    ...
    },
    {
      ...
      severity: 'CRITICAL',
      description: 'IP rate limit breached after 5 consecutive password mismatches. Account set to force-locked status.',
    ...
    },
    {
      ...
      severity: 'INFO',
      description: 'Spawned API live credential with scope list: ["read:users", "write:deployments"].',
    ...
    },
    {
      ...
      severity: 'WARNING',
      description: 'Enqueued password reset token inside background worker (BullMQ SMTP resend processor).',
      ipAddress: '198.51.100.42',
    ...
    <div className="p-6 bg-[#020617] h-full overflow-y-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Real-time log stream showing administrative actions, security transitions, and operational event trails.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 bg-[#0f172a] border border-[#1e293b] p-3 rounded-lg max-w-5xl">
        ...
        <div className="flex items-center space-x-2 shrink-0">
          ...
          <span className="text-[10px] text-slate-400 font-mono font-bold uppercase tracking-wider">Severity:</span>
      ...
      <div className="space-y-4 max-w-5xl">
          ...
          >
            ...
            <div className="flex-grow space-y-1 min-w-0 font-sans">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-mono text-xs font-bold text-white break-all">{log.action}</span>
                <span className={`text-[8px] uppercase font-mono font-bold tracking-wider px-1.5 py-0.2 rounded ${
                  log.severity === 'CRITICAL' 
                ...
                <span className="text-[10px] text-slate-500 font-mono ml-auto shrink-0">{log.createdAt}</span>
              </div>
              ...
              <div className="pt-2 flex flex-wrap gap-4 text-[10px] text-slate-500 font-mono border-t border-[#1e293b]/40 mt-1.5">
                <div>IP: <span className="text-slate-350">{log.ipAddress}</span></div>
                <div className="truncate max-w-xs md:max-w-md">UA: <span className="text-slate-350">{log.userAgent}</span></div>
              </div>
            </div>
        ...
        {filteredLogs.length === 0 && (
          <div className="p-12 border border-dashed border-[#1e293b] rounded-xl text-center space-y-2 text-slate-500 max-w-5xl select-none">
            ...
            <p className="text-[10px] text-slate-400 font-sans">Expand metrics scopes, cancel search key characters or reset filter tabs to view logs.</p>
          </div>
        )}
      </div>
